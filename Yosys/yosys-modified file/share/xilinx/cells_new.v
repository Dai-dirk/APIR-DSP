    ...
    generate
       always @ (*) begin
		case (CHAINMODEr)
			...
			2'b10: begin
				ACOUT = {{9'b0_0000_0000},{X_MUX[17:0]}};
			end
			2'b11: begin
				ACOUT = 18'bx;
			end
...
always @(*) begin
	case (MULTMODEr[3:2])
		mode_27x18: begin
			...
			A_sign_0 = 1'b0;
			A_sign_1 = 1'b0;
			A_sign_4 = MULTMODEr[0];
			A_sign_2 = 1'b0;
			A_sign_3 = 1'b0;
			A_sign_5 = MULTMODEr[0];

			B_sign_0 = 1'b0;
			B_sign_1 = 1'b0;
			B_sign_4 = 1'b0;
			B_sign_2 = MULTMODEr[1];
			B_sign_3 = MULTMODEr[1];
			B_sign_5 = MULTMODEr[1];
		end
		default: begin
			mode_SIMD = 1'b1;
			A_sign_0 = MULTMODEr[0];
			A_sign_1 = MULTMODEr[0];
			A_sign_4 = MULTMODEr[0];
			A_sign_2 = MULTMODEr[0];
			A_sign_3 = MULTMODEr[0];
			A_sign_5 = MULTMODEr[0];

			B_sign_0 = MULTMODEr[1];
			B_sign_1 = MULTMODEr[1];
			B_sign_4 = MULTMODEr[1];
			B_sign_2 = MULTMODEr[1];
			B_sign_3 = MULTMODEr[1];
			B_sign_5 = MULTMODEr[1];
		end
...
wire FULL = (MULTMODEr[3:2] == mode_27x18);
wire HALF_0 = (MULTMODEr[3:2] == mode_sum_9x9);
wire HALF_1 = (MULTMODEr[3:2] == mode_sum_4x4);
wire HALF_2 = (MULTMODEr[3:2] == mode_sum_2x2);
...
wire [44:0] M = C_0_shifted + C_1_shifted + C_2_shifted + C_3_shifted + C_4_shifted + C_5_shifted;
     ...
     wire [47:0] S = X ^ Y ^ Z_muxinv;
        ...
        generate
            if (USE_SIMD == "2X2") begin
                assign maj_xyz_simd_gated = {
                        maj_xyz_gated[47:41],
                        1'b0, maj_xyz_gated[39:35],
                        1'b0, maj_xyz_gated[33:31],
                        1'b0, maj_xyz_gated[29:27],
                        1'b0, maj_xyz_gated[25:23],
                        1'b0, maj_xyz_gated[21:17],
                        1'b0, maj_xyz_gated[15:13],
                        1'b0, maj_xyz_gated[11:0],
                        alu_cin
                    };
                assign int_carry_in[7:1] = 7'b000_0000;
                assign ext_carry_out = {
                        int_carry_out[7],
                        maj_xyz_gated[40] ^ int_carry_out[6],
                        maj_xyz_gated[34] ^ int_carry_out[5],
                        maj_xyz_gated[30] ^ int_carry_out[4],
                        maj_xyz_gated[26] ^ int_carry_out[3],
                        maj_xyz_gated[22] ^ int_carry_out[2],
                        maj_xyz_gated[16] ^ int_carry_out[1],
                        maj_xyz_gated[12] ^ int_carry_out[0]
                    };
                assign carryout_reset = 8'b0000_0000;
            end else if (USE_SIMD == "4X4") begin
                assign maj_xyz_simd_gated = {
                        maj_xyz_gated[47:35],
                        1'b0, maj_xyz_gated[33:27],
                        1'b0, maj_xyz_gated[25:17],
                        1'b0, maj_xyz_gated[15:0],
                        alu_cin
                    };
            ...
            assign {int_carry_out[7], alu_sum[47:41]} = {1'b0, maj_xyz_simd_gated[47:41]} + xor_xyz_muxed[47:41] + int_carry_in[7];
            assign {int_carry_out[6], alu_sum[40:35]} = {1'b0, maj_xyz_simd_gated[40:35]} + xor_xyz_muxed[40:35] + int_carry_in[6];
            assign {int_carry_out[5], alu_sum[34:31]} = {1'b0, maj_xyz_simd_gated[34:31]} + xor_xyz_muxed[34:31] + int_carry_in[5];
            assign {int_carry_out[4], alu_sum[30:27]} = {1'b0, maj_xyz_simd_gated[30:27]} + xor_xyz_muxed[30:27] + int_carry_in[4];
            assign {int_carry_out[3], alu_sum[26:23]} = {1'b0, maj_xyz_simd_gated[26:23]} + xor_xyz_muxed[26:23] + int_carry_in[3];
            assign {int_carry_out[2], alu_sum[22:17]} = {1'b0, maj_xyz_simd_gated[22:17]} + xor_xyz_muxed[22:17] + int_carry_in[2];
            assign {int_carry_out[1], alu_sum[16:13]} = {1'b0, maj_xyz_simd_gated[16:13]} + xor_xyz_muxed[16:13] + int_carry_in[1];
            assign {int_carry_out[0], alu_sum[12:0]} = {1'b0, maj_xyz_simd_gated[12:0]} + xor_xyz_muxed[12:0] + int_carry_in[0];
...
`ifndef YOSYS
        ...
        if (SEL_MASK != "MASK")     $fatal(1, "Unsupported SEL_MASK value");
        if (SEL_PATTERN != "PATTERN") $fatal(1, "Unsupported SEL_PATTERN value");
        if (USE_SIMD != "27X18" && USE_SIMD != "9X9" && USE_SIMD != "4X4" && USE_SIMD != "2X2")    $fatal(1, "Unsupported USE_SIMD value");
        if (IS_ALUMODE_INVERTED != 4'b0) $fatal(1, "Unsupported IS_ALUMODE_INVERTED value");
        if (IS_CARRYIN_INVERTED != 1'b0) $fatal(1, "Unsupported IS_CARRYIN_INVERTED value");
        if (IS_CLK_INVERTED != 1'b0) $fatal(1, "Unsupported IS_CLK_INVERTED value");
        if (IS_INMODE_INVERTED != 5'b0) $fatal(1, "Unsupported IS_INMODE_INVERTED value");
...
`endif
    ...
    wire signed [53:0] A_muxed;
    wire signed [53:0] B_muxed;
    ...
    reg signed [53:0] Ar1, Ar2;
    reg signed [26:0] Dr;
    reg signed [53:0] Br1, Br2;
    reg signed [47:0] Cr;
    reg        [4:0]  INMODEr;
    reg        [8:0]  OPMODEr;
    reg        [3:0]  ALUMODEr;
    ...
    // A/D input selection and pre-adder
    wire signed [53:0] Ar12_muxed = INMODEr[0] ? Ar1 : Ar2;
    wire signed [53:0] Br12_muxed = INMODEr[4] ? Br1 : Br2;

    wire INMODEA = (PREADDINSEL == "A") ? INMODEr[1] : 0;
    wire INMODEB = (PREADDINSEL == "B") ? INMODEr[1] : 0;

    wire signed [53:0] A2A1 = ( ~INMODEA ) ? Ar12_muxed : 0;
    wire signed [53:0] B2B1 = ( ~INMODEB ) ? Br12_muxed : 0;

    wire signed [26:0] Dr_gated   = INMODEr[2] ? Dr : 27'b0;

    wire signed [53:0] PREADD_AB = (PREADDINSEL == "A") ? A2A1 : B2B1;
    wire signed [53:0] AD_result  = INMODEr[3] ? (Dr_gated - PREADD_AB) : (Dr_gated + PREADD_AB);

    reg  signed [53:0] AD_DATA;
    generate
        if (ADREG == 1) initial AD_DATA = 54'b0;
        if (ADREG == 1) begin always @(posedge CLK) if (RSTD) AD_DATA <= 54'b0; else if (CEAD) AD_DATA <= AD_result; end
        else            always @* AD_DATA <= AD_result;
    endgenerate

    // 27x18 multiplier
    wire signed [53:0] A_MULT = (AMULTSEL == "A") ? A2A1 : AD_DATA;
    wire signed [53:0] B_MULT = (BMULTSEL == "B") ? B2B1 : AD_DATA;
...
`endif
            ...
            2'b11: X = $signed({Ar2[29:0], Br2[17:0]});
            default: X = 48'bx;
        endcase
...
`endif
   ...
   // reg signed [17:0] Br1, Br2;
    ...
    //B_MUX
    assign X_MUX[17:0] = (BREG == 1) ?  B_muxed[17:0]:B1[17:0];
    assign X_MUX[47:18] = X_MUX_A;
    // A and B cascade
	...
	2'b01: begin
	ACOUT = {{9'b0_0000_0000},{B1[26:18],B2[26:18]}};//B1B0_stream is B1/B2 MSB
	end
	2'b10: begin
	ACOUT = {{9'b0_0000_0000},{X_MUX[17:0]}};//B_MUX IS X_MUX[17:0]
	end
	2'b11: begin
	ACOUT = 18'bx;
	end 
